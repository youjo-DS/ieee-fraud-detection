...
split_test_index = [test[test['DT_M'] == month].index for month in test['DT_M'].unique()]
...
for i in range(NFOLD):
    ...
    models.append(model)  # for test predict
...
start_time = time()
for i in range(len(split_test_index)):
    for j in range(NFOLD):
        test_preds[split_test_index[i]] += models[j].predict(X_test.loc[split_test_index[i]]) / NFOLD
print(f'test predict finished in {str(datetime.timedelta(seconds=time() - start_time))}')
